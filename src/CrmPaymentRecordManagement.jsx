...
import { Table, Button, Modal, Form, Input, Select, InputNumber, DatePicker, message, Space, Popconfirm, Card } from 'antd';
import { PlusOutlined, ReloadOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
import api from './auth';
import dayjs from 'dayjs';
import EntityPicker from './components/EntityPicker';

const ENTITY = 'payment-records';

export default function CrmPaymentRecordManagement({ user, companies = [] }) {
  ...
  const columns = [
    ...
    { title: '回款金额', dataIndex: 'record_amount', width: 130, align: 'right', render: v => v != null ? Number(v).toLocaleString() : '-' },
    { title: '回款日期', dataIndex: 'record_date', width: 120, render: v => v || '-' },
    { title: '付款方式', dataIndex: 'payment_method', width: 130, render: v => v || '-' },
    { title: '备注', dataIndex: 'remark', ellipsis: true, render: v => v || '-' },
    { title: '操作', key: 'actions', width: 150, fixed: 'right', render: (_, r) => (
      <Space>
        <Button size="small" icon={<EditOutlined />} onClick={() => openEdit(r)}>编辑</Button>
        <Popconfirm title="确认删除该回款记录?" okText="确认" cancelText="取消" okButtonProps={{ danger: true }} onConfirm={() => handleDelete(r.id)}>
  ...
  return (
    <Card title="回款记录管理" extra={isSuperAdmin && (
      <Select placeholder="选择公司" style={{ width: 180 }} value={selectedCompanyId}
        onChange={v => { setSelectedCompanyId(v); setPage(1); }}
        options={(companies || []).map(c => ({ label: c.name, value: c.id }))} />
    )}>
      <Space style={{ marginBottom: 16 }}>
        <Input.Search placeholder="搜索付款方式/备注" value={keyword} onChange={e => setKeyword(e.target.value)} onSearch={fetchData} style={{ width: 240 }} allowClear />
      ...
      <Modal title={editing ? '编辑回款记录' : '新增回款记录'} open={modalOpen} onOk={handleSave} width={560}
        ...
        <Form form={form} layout="vertical">
          <Space wrap>
            ...
            <Form.Item name="payment_plan_id" label="回款计划"><EntityPicker entity="payment-plans" companyId={effectiveCompanyId || 0} placeholder="搜索回款计划" width={280} labelField="stage_name" /></Form.Item>
          </Space>
          <Space wrap>
            <Form.Item name="record_amount" label="回款金额"><InputNumber style={{ width: 200 }} min={0} step={0.01} /></Form.Item>
            <Form.Item name="record_date" label="回款日期"><DatePicker style={{ width: 200 }} /></Form.Item>
            <Form.Item name="payment_method" label="付款方式"><Input style={{ width: 200 }} /></Form.Item>
          </Space>
